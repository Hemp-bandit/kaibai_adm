...
export async function get_access_list(data: GetAccessListReqData) {
  return await user_instance.post<any, T_basic_rsp<ListResponse<AccessData>>>('/access/get_access_list', data)
}

export async function createAccess(data: CreateAccessData) {
  return await user_instance.post<any, T_basic_rsp<string>>('/access/create_access', data)
}

export async function updateAccess(data: UpdateAccessData) {
  return await user_instance.post<any, T_basic_rsp<string>>(`/access/update_access`, data);
}

export async function deleteAccess(id: number) {
  return await user_instance.delete<any, T_basic_rsp<string>>(`/access/${id}`);
}

export async function getAccessOption() {
  return await user_instance.get<any, T_basic_rsp<AccessOption[]>>(`/access/access_map`);
}
